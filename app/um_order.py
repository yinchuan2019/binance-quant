...
# if c.isTest:
#     client = CMFutures(api_key=c.api_key_test, api_secret=c.api_key_test, kwargs={"base_url": c.FUTURE_URL_TEST})
# else:
#     client = CMFutures(api_key=c.api_key, api_secret=c.api_secret)
#
...
#         try:
#             client.exchange_info(self.symbol)
#             self.exchangeRule = exchangeInfo(dict['symbols'])
#
#             # 获取K线数据
...
#
#                     if isToBuy is True:
#                         ud_account = client.account(self.quote_asset)
...
#
#                         print(f'现货资产 {balances[0]["asset"]}:{balances[0]["free"]}')
#
#                         # 查询当前价格
#                         price = client.ticker_price(self.symbol)
#                         # 购买量
#                         count = self.format_trade_quantity(balance / float(price))
#
#                         order_params = {}
...
#                         balances[0]["asset"]
#
#                         quantity = self.format_trade_quantity(float(balances[0]["free"]))
#                         # 查询当前价格
#                         # curprice = client.get_ticker_price(self.symbol)
#                         if quantity <= 0:
